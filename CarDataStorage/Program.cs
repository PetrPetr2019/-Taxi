...
namespace CarDataStorage
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                new Truck{ModelAvto = "Камаз", Carrying = 20, DataofIssue = 1999, DataofMajorOverhaur = 2015, NumberGos = 374532, EncryptPark = 23},
                new Truck{ModelAvto = "Белаз", Carrying = 200, DataofIssue = 2000, DataofMajorOverhaur = 2018, NumberGos = 34853, EncryptPark =12}
            };
            List<Taxi> taxis = new List<Taxi>
            {
              new Taxi{MarkaAvto = "Audi - 6", NumberPassengerSeat = 5, Data = 2000, GosNUmber = "С 065MK-78rus", EncryptPark = 55},
              new Taxi{MarkaAvto ="BMW", NumberPassengerSeat = 4, Data = 2017, GosNUmber = "АН 7331-47КRus", EncryptPark = 45},
            ...
            var Query = from Trusks  in  trucks
                        join T_autopark in  taxis on Trusks.ID equals T_autopark.ID
                        select new
                        {
                            Trusks.ModelAvto,
                            T_autopark.MarkaAvto,
                            T_autopark.NumberPassengerSeat,
                        };
            foreach (var rez in Query)
            {
                Console.WriteLine("\n{0}-{1}-{2}", rez.ModelAvto, rez.NumberPassengerSeat, rez.MarkaAvto);
            }
            Console.ReadLine();
